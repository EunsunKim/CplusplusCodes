//This is a sample of smart pointers cpp. 
//Two more header files are required to run.
...
/* Each element in that list is of Product type.
Your function compares elements in the two received lists for common product codes
and builds the user-friendly list from the matching pairs.
Your function allocates dynamic memory for each user-friendly element
and adds that element to the new list by passing the object's raw pointer 
to the list's += operator.  */
	...
	return priceList;   //This function returns the user-friendly price list.
}
